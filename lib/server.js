...
const Encryption = require("./Encyption");
...
app.post("/get-balance", async (req, res) => {
    try {
        const { walletPrivate, address } = req.body;
        ...
        const provider = new ethers.JsonRpcProvider(tokenNet.value === 'live_net' ? process.env.NODE_RCP_URL : process.env.NODE_TEST_RCP_URL);
...
app.post("/get-balance-by-address", async (req, res) => {
    try {
        const { walletPrivate, contractAddress } = req.body;
...
app.post("/transfer-bnb", async (req, res) => {
    try {
        const { amount, toAddress, walletPrivate } = req.body;
        ...
        // if (tx.status === 1) {
            res.status(200).json({ data: tx, amount, toAddress, contractAddress: 'binance' });
        // }
    } catch (e) {
        // console.log(err, '\n\n');
        // console.log(err.stack);
...
app.post("/transfer-fees", async (req, res) => {
    try {
        ...
        const tx = await transaction.wait();
        // if (tx.status === 1) {
            res.status(200).json({ data: tx, amount: ethers.formatEther(amount), toAddress, contractAddress: 'binance' });
        // }
    } catch (e) {
        // console.log(err, '\n\n');
        // console.log(err.stack);
...
app.post("/transfer-token", async (req, res) => {
    try {
        const { toAddress, walletPrivate, contractAddress } = req.body;
